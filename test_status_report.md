...
**(軽微)必要規模が足りていなくてもグレーアウトされていない**
 → マリガンで引いたカードなどものによっては判定を逃れてしまう

**(要望)マリガン後の確認ビューにモーダルを閉じるまでのカウントダウン表示が欲しい**

**(要望)ゲーム終了画面に、ターンごとの意識推移チャートを出したい**
 →　各プレイヤーの各ターン終了時の意識を記録しておく
 →　自分と相手の意識の推移を比較できるように折れ線を出す

**(要検証)大地震の効果が場合によって多重になっている！？**
 →　そもそも引いたとき効果が発動してなくない？→プレイヤーが引いたとき上手くいってたなぁ……なぜ？(終末もプレイヤーが引いたときは大丈夫だった)
...
2.  **カード効果の処理順序の制御**
    *   **現象**: ターン開始・終了時に複数のカード効果が発動する場合の順序が意図通りでない。現在は"ターン中のプレイヤーを先に処理する"という方針に沿っておらず、プレイヤーid順(カードの順序は問題ない)
    *   **方針**: ルールに基づいて配列にデータを格納する順を制御する仕組みを導入します。
