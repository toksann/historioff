...
## 備考
- その他気になること
    - ターン開始時や終了時のカードの処理順を調査。可能であれば制御したい(「場のイデオロギー→場の財の配置が古い順→手札→デッキ→捨て札」これをターン中のプレイヤー→相手の順)
...
## バグ分析と修正方針
**モバイルUIの調整**
 → デッキ構築画面 : ipadAir -> 縦横どちらでもデッキ領域は見えるがドラッグアンドドロップがフリックでは上手くいかず、カードを長押ししてから引っ張らないといけない
                iphoneSE -> デッキリストとデッキアクションの反転、規模分布から「規模分布」をなくす。切替ボタンも無くして、グラフをボタン化して押したら切替できるようにする。区間ラベル1桁
 → ゲーム画面： ipadAir -> 横向きのときは縦が足りない。縦向きのときは横が足りない。
            iphoneSE -> カードが空カードとサイズが合っておらず大きすぎる。
 → 共通項：とにかく要素がデカい。領域もデカい。のでメディアクエリで小さくしていく必要がある。
 → というかそもそも画面のピクセル数いくつなのスマホって？ → 950未満と600未満
...
**(調整済み)一部のアニメーションを調整したい**
 ...
 → しかし、ダメージ→破壊に関しては、ダメージ演出が出ず破壊だけ出てしまう(根深そう。調査難航)

**(要検証)大地震の効果が場合によって多重になっている！？**
 →　とりあえず、自分が使っても、相手が使っても大丈夫。検証が難航すると思われるので、ひとまず気にせずパッケージングまで持っていこう
