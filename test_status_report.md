...
| ポピュリズム | test_populism_effect.js | 成功 | 自動配置効果は手札に無い場合は発生していない & 自動配置された場合、画面から見えなくなっている(その場にはあるらしい) & イデオロギー選択効果が自動配置した場合は働かない |
...
6.  **カードの状態・位置に関するバグ**
    *   **対象カード**:`ポピュリズム`
    *   **現象**:
        *   `ポピュリズム`: 自動配置(手札にあるとき以外失敗)、配置時効果、複数の効果が機能不全。「テスト状況」に詳細あり。
