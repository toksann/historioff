...
**一部のアニメーションを調整したい**
 → ターン関連は終了→開始の流れを確認して順序が合うようにしたい。財のダメージ/強化&場から移動は、ターン系も含めて再生が重ならないようにしないように待たせる。
 → ターン開始はこれが終わるまで対象プレイヤーが操作できないようにする

**意識推移チャートにアニメーション付けれるのでは？**
...
**(保留)新アニメーションが現状微妙なので、調整してみる or 破棄する**
 → とりあえずブランチ分けた
