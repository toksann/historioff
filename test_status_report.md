 ...
 **'enforceFieldLimit使ってなくね？→有用なら使う、要らないならただ消す**
 **公開効果をログに乘るようにしたい**
 **カード入手時の選択画面にもカード詳細確認ボタンを置きたい**
 **サレンダーできるようにしたい**
...
2.  **カード効果の処理順序の制御**
    ...
    *   **方針**: 備考欄の希望仕様（イデオロギー→場の古い順→手札…）を参考に、処理対象のカードを一度リストに集め、ルールに基づいてソートしてから順に効果を処理する仕組みを導入します。
